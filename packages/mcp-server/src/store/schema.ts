 ...
 * merged in here too, so `db._.fullSchema` (the drizzle adapter) and
 * `drizzle-kit generate` (the committed ./drizzle SQL applied by both dialects) see
 * ONE combined schema — one migration set, one Postgres. `grants.sub` FKs
 * `user.id`, and drizzle-kit sequences the auth tables (0000) before the tenancy
 * tables (0001), so the FK never fires against a missing `user`.
 */
