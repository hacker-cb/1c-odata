...
The keychain service is now `1c-odata:<first 16 hex of sha256(canonical data dir)>`,
with the connection name kept as the human-readable account. Two clients resolving
the **same** data dir compute the same service and keep sharing (the
agent-independent default-dir model is preserved); two **different** dirs isolate.
