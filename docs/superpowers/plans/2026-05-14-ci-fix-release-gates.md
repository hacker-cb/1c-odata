...
NB: это первый push в новый remote `hacker-cb/1c-odata` любой не-master ветки. CI на новый remote запустится; если все 5 ожидаемых секретов (`ONEC_TRADE_V11_5_URL`, `ONEC_BP_V3_0_URL`, `ONEC_EXAMPLE_BASIC_URL`, `HTTP_PROXY`, `ONEC_TESTS_ALLOW_WRITES`) уже настроены — пройдут все 8 jobs включая live/write/example. Если каких-то секретов нет — `detect-secrets` упадёт явно с перечислением missing names и поведёт за собой `test-live`/`test-write`/`test-example` через `needs:`-edge; `lint-and-typecheck` + `test-and-build` останутся независимыми и нам важны именно они.
...
Если файл пустой → все jobs прошли, перейти к Task 6.
